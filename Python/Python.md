# Python
* ORM : Object relational mapping, 데이터베이스와 객체 지향 프로그래밍 언어 간의 호환되지 않는 데이터를 변환하는 프로그래밍 기법, 객체 지향 언어에서 사용할 수 있는 “가상” 객체 데이터베이스를 구축하는 방법이다
* 프로토콜(어떻게), 트랜스포트(어떤), 프로퍼티 (파이썬의 게터와 세터)
* 컴퓨터끼리 통신하려면 양쪽 컴퓨터 모두 미리 정의된 프로토콜 모음을 사용해야 한다
* 프로퍼티 - 필드, 세터와 게터를 만들어준다, 세터와 게터의 단점을 보완해줄 수 있다 (무분별한 게터와 세터의 생성)
* 튜플 (3개 이상의 타입을 사용할 때, 반환값이 3개 이상)
* 네임스페이스 : 변수에 객체를 할당하면, 변수와 변수에 연결된 딕셔너리 형태로 연결되어 네임스페이스 안에 저장된다 (패키지, 이름 충돌을 막기 위해)
...
* 대부분의 직접 만든 설정 파일은 지저분하고, 빠르지 않다, writer 프로그램과 reader 프로그램 모두 관리해야하므로 오류가 날 확률이 높다
...
# 관계형 데이터베이스
* 다수의 동시 사용자가 데이터에 접근 할 때 사용한다
* 사용자에 의한 데이터 손상으로부터의 보호 할 때 사용한다
* 데이터를 저장하고 검색하는 효율적인 방법이다
* 스키마에 정의된 데이터와 제약조건에 한정되는 데이터이다
...
# 발행 - 구독 모델
* 발행 - 구독은 큐가 아닌 브로드캐스트이다, 하나 이상의 프로세스가 메시지를 발행한다
...
# 서버 자동화
* 솔트 : 주 설계 목표는 속도이다
* 앤시블 : 클라이언트 시스템에 파이썬만 설치되어 있어도 충분하다
* 퍼핏 : 루비로 작성되었으며, 구성을 위해 퍼핏 스크립트라는 자체 언어를 제공한다
* 셰프 : 인프라구조 코드 작성 언어로 루비를 사용하는 경우 대체로 구성 관리에 셰프를 사용한다
* CFEngine : C로 작성되었기에 매우 작은 풋프린트를 가진다, 오류에 대한 견고함을 달성하기 위해, 분산 네트워크에서 작동하는 자율 에이전트를 사용한다

# 시스템 / 작업 모니터링
* Psutil : 유닉스 유틸리티 함수로 얻을 수 있는 정보를 파이썬으로 제공한다, 서로 다른 시스템 정보에 대한 크로스 플랫폼 인터페이스이다
* Fabric : SSH를 통해 원격 호스트 목록에서 명령을 쉽게 정의하고 실행할 수 있다, 시스템 관리 작업을 단순화하기 위한 라이브러리로 SSH를 사용해서 여러 호스트에 연결하고 각 호스트에서 작업을 실행할 수 있다
* Luigi : 연쇄 하둡 명령과 같이 장시간 실행되는 프로세스를 스케줄링하고 모니터링할 수 있다, 직접 작성하고자 하는 어떠한 작업이든 결합하여, 대규모로 장시간 실행되는 배치 작업의 파이프라인 전체를 관리할 수 있도록 돕는다

# 스레딩
* 파이썬의 threading 라이브러리를 사용하면 여러 스레드를 생성할 수 있다
* 전역 인터프리터 잠금 때문에 하나의 파이썬 인터프리터는 하나의 파이썬 프로세스만 실행한다
* 하나 이상의 스레드가 블로킹되는 경우에만 성능 향상이 가능하다
* 파이썬에서는 여러 스레드가 있을 떄, 하나의 스레드가 I / O 때문에 블로킹되면, 다음 스레드가 프로세서를 사용하도록 전환한다
...
# JSON 파싱
* JSON은 말 그대로 자바스크립트에서 객체를 정의하는 데 사용하는 표기법이다
* json 라이브러리는 JSON 형식의 문자열이나 파일을 파이썬 딕셔너리로(혹은 리스트로) 파싱한다
...
* 경우에 따라 직렬화된 데이터의 크기를 최소화하여 디스크 필요 공간 혹은 대역폭 요구 정도를 최소화하는 것이 목적이 되기도 한다

# 네트워킹
* asyncio : 파이썬 소프트웨어 재단 라이센스, 비동기 이벤트 루프를 제공하여 비차단 소켓과 큐뿐만 아니라 사용자 정의 코루틴과의 통신관리, 비동기 소켓과 큐가 포함되어 있다
* gevent : MIT 라이센스, 비동기 I / O 를 위한 C 라이브러리인 libev와 긴밀히 결합됨
