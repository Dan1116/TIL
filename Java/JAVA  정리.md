...
* 자바는 class 를 통해서 인스턴스를 생성한다
* 업케스팅 후 재정의 된 메소드를 호출해야만 한다
* String은 불변 객체 (상태가 변경 되어야 한다면 그 상태를 가지는 새로운 객체를 생성해야 한다)
* 자식의 공통된 기능은 반드시 부모로부터 비롯되어야 한다 (부모의 레퍼런스를 통해서 자식의 기능을 온전히 이용할 수 있다)
* 자식은 반드시 기능을 제공해야 하고 부모가 구현을 제공할 필요가 없다면, 추상 메소드를 사용하는게 좋다
* 리팩토링 (Refactoring) : 기존 코드의 동작을 변경하지않고 구조를 개선하는 작업, 마틴 파울러, 코드의 유지보수성을 떨어트리는 요소 - 냄새를 제거 (smells) -> Replace type code with polymorphism
...
* protected 를 사용하면 안 되는 이유는 재사용이 불가능하기 때문이다
* 오버헤드 (Overhead)란 어떤 처리를 하기 위해 들어가는 간접적인 처리 시간, 메모리 등을 말한다
* 상태에 따라 만들려면 enum 을 사용해야 한다
...
## 방어적 복사
* 생성자에서 진행하는 인자의 유효성 검사는 방어적 본사본을 만든 후에 진행해야한다, 인자를 검사한 직 후 복사본이 만들어지기 직전까지의 시간 사이에 다른 스레드가 인자를 변경해 버리는 경우가 있어 이를 방지하기 위함이다
* 만약 생성자를 통해서 전달된 데이터가 변경 가능한 객체일 때 이를 복사하기 위해서 clone() 메소드를 호출하고 싶을 때는 해당 클래스가 final 로서 추가적으로 상속 가능한 클래스가 아니어야 한다, 추가적으로 상속할 수 있는 경우 해당 객체의 clone() 메소드를 실행 했을 때, 해당 객체가 반환된다고 보장할 수 없기 때문이다
* 생성자 이외에도 접근자들에 대해서 방어적 복사본을 반환하도록 해야한다, 값이 변경될 여지가 있기 때문이다

## Boxing
* Boxing : Collection 에 Primitive Type 을 저장하기 위해서는 Reference Type 으로 변환해야 한다
* 자동으로 박싱을 하는 것을 Auto Boxing 이라 한다
...
* 객체 생성의 과정은 동일하지만, 다른 표현을 가지는 객체를 생성할 때 사용하는 패턴이다
* 불변 객체는 새로운 표현마다 새로운 객체가 만들어진다
* 프로그램 성능은 객체의 생성과 파괴에 큰 영향을 받는다
* String 과 다르게 불변 객체가 아니다
* 문자열 연산이 자주 있을 때 사용한다
* Builder 는 단일 쓰레드 환경에서 사용하기 좋다
* Buffer 는 멀티 쓰레드 환경에서 사용하기 좋으며 동기화를 지원, =을 쓸 이유가 없다

##  @Override (런타임 다형성을 구현하는 기능)
* 컴파일러에게 특정한 동작을 한다는 것을 알려주는 기능이다 (Annotation)
* 부모로부터 상속받은 클래스는 앞에 override를 명시한다
* 런타임에 작동하는 annotation을 만들어야 한다 (build, FunctionalInterface)

## 정적 배열 Array
* 컴파일 타임에 크기가 결정된다
* 사이즈는 초기화시 고정된다
* 초기화시 메모리에 할당, 속도가 빠르다
* 사이즈 변경 불가하다
...
* 초기화시 사이즈를 표시하지 않는다, 유동적이다
* 추가 시 메모리를 재할당해야하므로, 속도가 느리다
* 추가 삭제 기능이 있다 (add ( ), remove ( ))
...
* 원시 데이터 유형은 메모리 주소로 참조 될 수 없다, 그렇기 때문에 원시 값의 자리 표시자 역할을 하는 래퍼가 필요하다, 이 값들은 돌연변이되어 액세스되거나, 재구성되거나, 정렬되거나 무작위화 될 수 있다
...
## 패키지를 쓰는 이유
* 이름 충돌을 방지하기 위해서 사용한다, 소속 회사의 도메인을 거꾸로 사용하는게 보편적
...
Reference 타입으로 만드는 방법  : class, Array, enum, interface

## SOLID 객체지향 설계 5대 원칙
* Uncle Bob (Robert C Martine = clean code)

### SRP(Single Responsibility Principle)
: 단일 책임의 원칙
-> 모듈 (변수, 함수, 클래스, 프로그램)은 단 하나의 책임을 가져야 한다
...
* 경쟁 조건이 발생하였을 때, 정수의 연산만 존재한다면, 뮤텍스보다는 다른 방법을 쓰는 것이 좋다 : 원자적 연산(CPU 명령)
...
* 인자의 전달은 stack을 이용한다
* 함수 호출 이전의 상태와 함수 호출 후의 스택 상태는 동일해야 한다
* 가변 인자는 스택을 통해 인자가 전달된다 (가변 인자를 사용하려면 고정 인자 (가변인자 바로 앞에 위치한 인자)가 필요하다)
...
* 스레드 사이에서 공통적으로 사용해야 할 객체가 있다면 동기화에 대해 고민을 해야 한다
* 다중 스레드 작업 시에는 각 스레드 끼리 정보를 주고 받을 수 있어 처리 과정의 오류를 줄일 수 있다, 프로세스 끼리는 정보를 주고 받을 수 없다
