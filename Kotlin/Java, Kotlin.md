...
## 블록
*  불필요한 CPU를 점유하지 않는다 
* 이벤트가 발생하기 전까지 아무것도 수행할 수 없다
* 블록 논 블록의 차이는 이벤트에 대해 스레드나 프로세스가 잠을 자는지 안 자는지의 차이로 생각하는게 이해하기 좋다

## 동기  
* 자신이 직접 수행한다
* 순서 제어가 용이하다
* 함수의 반환 값이나 완료를 통해 완료 시점을 알 수 있다 

## 비동기
* 병행성에 좋다
* 실행 시점을 알 수 없고 순서를 제어하기 어렵다
* 별도의 스레드나 프로세스를 통해 일을 처리한다
* 완료 시점을 알 수 없으므로 콜백을 꼭 사용해야한다

* 대부분의 코드는 call by value 로 변경해야 한다
* detach : 적절하게 사용하지 않으면 누수가 발생할 수 있다
* 스레드가 관여하는 것에는 mutex 를 사용해야한다

### 결합
* 인터페이스나 추상 클래스에 의존 하는 것이 약한 결합이다
* 특정한 객체에 의존 하면 강한 결합이라 부른다
...
### Heap 에 해시코드 값이 저장된다
* 해시 : 버킷의 크기가 무한정하지 않다, 다른 값이여도 동일한 해시 값이 나올 수 있다
...
* 다양한 언어를 지원한다
* gRPC  (Remote Procedure Call) : 모든 언어에 사용가능하다
...
* 동일한 시그니처를 가진 동일한 함수는 공존이 불가능하다 (이름 충돌) -> 패키지를 분리시켜야 한다
* 일반 함수를 메소드처럼 사용하게 만들어 주는 방법이다
* 가독성을 높히기 위해 사용한다
* 코틀린 라이브러리의 핵심 , 모든 라이브러리는 확장함수로 되어있다
* 상속을 통해 기능을 확장하는 것이 아니라 인터페이스의 부분을 확장하는 기법이다
...
* 문자열에 관해서 트림(trim, 공백 잘라내기)이라는 부분을 체크해줘야한다
...
### 로깅
* 기록
* 링 버퍼 : 버퍼의 용량이 초과되면 처음부터 다시 쓰는 방법이다

### 함수 합성
* 기존 함수를 합성해서 새로운 함수를 만드는 기법이다
* bind : 어떠한 타입이 될지 이미 결정되었다

## 고차 함수
* 매개변수 또는 반환 값으로 또 다른 함수를 갖는 함수를 고차함수라도 부르는데, 자바에서는 콜백함수로 불린다

### 함수, 클로저
* 고차함수 : 함수를 인자로 받거나 함수를 반환하는 함수이다
* 클로저 : 고차함수가 반환하는 함수를 뜻한다
* 익명함수 : 이름 없이 임시로 쓰는 함수이다
* 람다함수(람다식) : 익명함수의 또다른 이름이다

### 코들린의 object
* 싱글턴을 정의할 때 사용한다
* 동반객체를 이용한 팩토리 메서드를 구현할 때 사용한다
* 무명클래스의 선언을 할 때 사용한다
* object 로 선언하면 클래스 선언과 동시에 객체가 생성된다

## @JvmOverloads
* 코틀린은 @JvmOverloads라는 고유한 어노테이션을 통해 간결한 생성자 오버로딩을 지원한다
* 어노테이션은 컴파일러에게 인수를 default 값으로 대체하는 생성자를 만들도록 지시한다
