...
* 자바와 비교하면 코틀린의 타입 시스템은 코드의 가독성을 향상시키는데 도움이 되는 몇 가지 특성을 새로 제공한다
...
* let을 사용하는 가장 흔한 사용법은 널이 될 수 있는 값을 널이 아닌 값만 인자로 받는 함수에 넘기는 경우다
...
### 나중에 초기화할 프로퍼티
* 코틀린에서 클래스 안의 널이 될 수 없는 프로퍼티를 생성자 안에서 초기화하지 않고 특별한 메소드 안에서 초기화 할 수는 없다
...
* 이런 파라미터 값 검사는 함수 내부에서 파라미터를 사용하는 시점이 아니라 함수 호출 시점에 이뤄진다, 따라서 가능한 빨리 예외가 발생하기 때문에 원인을 파악할 수 있다
...
* Collection에는 원소를 추가하거나 제거하는 메소드가 없다
...
* 자바에서 가져온 타입은 코틀린에서 플랫폼 타입으로 취급된다, 개발자는 플랫폼을 널이 될 수 있는 타입으로도, 널이 될 수 없는 타입으로도 사용할 수 있다
...
* Any 타입은 다른 모든 타입의 조상 타입이며, 자바의 Object에 해당한다, Unit 은 자바의 void 와 비슷하다
